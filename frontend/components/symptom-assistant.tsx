...
export function SymptomAssistant() {
      ...
      .catch((reason) =>
        setError(
          reason instanceof ApiError
            ? reason.message
            : "Symptom guidance could not load.",
        ),
      )
  ...
  if (loading)
    return (
      <section className="symptom-shell" aria-busy="true">
        Loading symptom guidance...
      </section>
    );
  if (!setup)
    return (
      <section className="symptom-shell" role="alert">
        {error}
      </section>
    );
  ...
  return (
    <section className="symptom-shell">
      <div className="symptom-intro">
        ...
        <p>
          This flow highlights urgency, red flags, and questions for a
          clinician. It does not diagnose or confirm conditions.
        </p>
      </div>
        ...
        <form className="symptom-form" onSubmit={submit}>
          ...
          <label>
            ...
            <input
              ...
              placeholder="2 days, since this morning"
              maxLength={120}
            />
          </label>
          <label>
          ...
          <label>
            ...
            <input
              ...
              placeholder="nausea, rash, dizziness"
            />
          </label>
          <label>
            Relevant context
            <textarea
              ...
              rows={4}
              maxLength={1000}
              placeholder="Recent travel, medicines, allergies, chronic conditions, or triggers"
            />
          </label>
          <fieldset>
            ...
            {warningOptions.map((option) => (
              <label key={option} className="check-row">
                <input
                  type="checkbox"
                  checked={warnings.includes(option)}
                  onChange={() => toggleWarning(option, setWarnings)}
                />
                {option}
              </label>
            ))}
        ...
        {history.length ? (
          history.map((item) => (
            <button key={item.id} onClick={() => setAssessment(item)}>
              ...
              <small>
                {item.urgency_level.replaceAll("_", " ")} -{" "}
...
function ResultPanel({ assessment }: { assessment: Assessment | null }) {
  if (!assessment) {
    return (
      <aside className="symptom-result empty">
        <h2>Guidance will appear here</h2>
        <p>Use this before a care conversation, not instead of one.</p>
      </aside>
    );
  }
  ...
  return (
    <aside className={`symptom-result urgency-${guidance.urgency_level}`}>
      <span className="urgency-label">
        {guidance.urgency_level.replaceAll("_", " ")}
      </span>
      <h2>Symptom guidance</h2>
      <p>{guidance.educational_explanation}</p>
      <h3>Possible categories of causes</h3>
      <ul>
        {guidance.possible_cause_categories.map((item) => (
          <li key={item}>{item}</li>
        ))}
      </ul>
      <h3>Red flags</h3>
      <ul>
        {guidance.red_flags.length ? (
          guidance.red_flags.map((item) => <li key={item}>{item}</li>)
        ) : (
          <li>No emergency red flags were selected or detected.</li>
        )}
      </ul>
      ...
      <ul>
        {guidance.doctor_questions.map((item) => (
          <li key={item}>{item}</li>
        ))}
      </ul>
      <h3>Safe self-care</h3>
      <ul>
        {guidance.safe_self_care.map((item) => (
          <li key={item}>{item}</li>
        ))}
      </ul>
      {guidance.citations.length > 0 && <h3>Citations</h3>}
      {guidance.citations.map((item) => (
...
function toggleWarning(
  option: string,
  setWarnings: (updater: (items: string[]) => string[]) => void,
) {
  setWarnings((items) =>
