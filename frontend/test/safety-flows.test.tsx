...
vi.mock("@/lib/api-client", () => ({ apiRequest: vi.fn(() => Promise.resolve([])) }));
...
describe("safety-critical public flows", () => {
  it("shows emergency action before long educational content", () => {
    ...
    expect(screen.getByRole("heading", { name: /chest pain/i })).toBeInTheDocument();
  });

  it("keeps misinformation checker framed around evidence", () => {
    render(<ToastProvider><MisinformationChecker /></ToastProvider>);
    expect(screen.getByRole("heading", { name: /paste a health claim/i })).toBeInTheDocument();
