...
export function SharingConsentCenter() {
  ...
  const [message, setMessage] = useState(
    "Only selected records are shared. Access is read-only and can be revoked anytime.",
  );
  ...
  async function createGrant() {
    ...
    setMessage(
      "Sharing grant created. The doctor can only read the selected records until expiry.",
    );
    await load();
  }
  ...
  async function revoke(id: string) {
    if (
      !window.confirm(
        "Revoke this doctor's access now? They will lose access immediately.",
      )
    )
      return;
  ...
  return (
    <section className="sharing-shell" aria-labelledby="sharing-title">
      <div className="sharing-hero">
        ...
        <h1 id="sharing-title">
          Share selected records with a verified doctor
        </h1>
        <p>{message}</p>
      </div>
      <div className="sharing-grid">
        <form className="sharing-card" action={createGrant}>
          ...
          <label>
            Selected record IDs, comma separated
            <textarea
              value={recordIds}
        ...
        <div className="sharing-card">
          ...
          {grants.length === 0 ? (
            <p>No active or past grants loaded.</p>
          ) : (
            grants.map((grant) => (
              <article key={grant.id} className="share-row">
                ...
                <span>
                  Status: {grant.revoked_at ? "Revoked" : "Active until expiry"}
                </span>
                {!grant.revoked_at && (
                  <button type="button" onClick={() => revoke(grant.id)}>
                    Revoke
                  </button>
                )}
              </article>
      ...
      <div className="sharing-grid">
        <div className="sharing-card">
          <h2>Doctor review notes visible to you</h2>
          {notes.map((note) => (
            <p key={note.id}>
              <b>Record {note.record_id}</b>: {note.note}
            </p>
          ))}
        </div>
        <div className="sharing-card">
          ...
          {audits.map((audit) => (
            <p key={audit.id}>
              {audit.action} {audit.record_id ? `- ${audit.record_id}` : ""}
            </p>
          ))}
        </div>
      </div>
